...
const ChiquvchiImzolashUchunContent=({ currentUser, permission, ranks })=> {
  ...
  const SearchData = async () => {
    try {
      const res=await axiosInstanceOut.post(`missive/list`, {
        confirmativeWorkPlace: tasdiqlovchiRef?.current?.value,
        correspondent: korrespondentRef?.current?.props.value.value,
        shortInfo: qisqachaMalumotRef?.current?.value,
        workPlace: JSON.parse(localStorage.getItem('ids')),
        size:20,
        page: selected,
        tab: 5,
  ...
  return (
    <div className="content mb-5 content-mobile">
      <h3 style={{ margin: "10px 0 0 20px", fontWeight: "bold", textTransform: "upperCase" }}>Imzolash uchun</h3>
                        ...
                        <td className="harakat">
                          <div>
                            ...
                            <div
                              ...
                              <Link
                                to={`/chiquvchi/ko'rish/imzolash_uchun/${data?.id}`}
...
export default React.memo(ChiquvchiImzolashUchunContent);