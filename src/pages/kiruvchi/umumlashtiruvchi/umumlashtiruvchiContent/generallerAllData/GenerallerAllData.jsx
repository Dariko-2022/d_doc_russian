...
const GenerallerAllData = ({ data, linkFunc }) => {
    ...
    <table className="table table-bordered my-1 table-striped table-hover Tab" id="myTable">
      ...
      <tbody>
        {data.content?.length > 0 && data.content.map((dat, index) => (
          <tr key={dat.id}
            onClick={() => window.innerWidth <= 768 && history.push(`/kiruvchi_bajarish_ijro/${dat?.id}/umumlashtiruvchi`)}
          >
...
export default React.memo(GenerallerAllData);