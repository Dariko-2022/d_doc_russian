...
const RejectedInputsElements = ({ setData, setSelected }) => {
  const [startDate, setStartDate] = useState();
  const korresref = useRef();
  const shortDescref = useRef();
  const regNumref = useRef();

  console.log("RejectedInputsElements");
  ...
  const All = useCallback(async () => {
    try {
      ...
      setData(res.data);
      setSelected(0);
      extensiveSearchBackPageId({ selected: 0, pageName: "redEtilganPageId" })
    } catch (error) {
...
export default React.memo(RejectedInputsElements);