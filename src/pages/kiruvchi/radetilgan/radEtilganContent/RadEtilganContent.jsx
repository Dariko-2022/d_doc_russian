...
import RejectedAllData from './rejectedAllData/RejectedAllData';
import ContentNavbar from "../../contentNavbar/ContentNavbar";
import { axiosInstance } from "../../../../config";
import RejectedInputsElements from "./rejectedAllData/RejectedInputsElements";
...
const RadEtilganContent = ({ currentUser, permission, ranks }) => {
  const { insta: user } = useSelector(state => state.user);
  const history = useHistory();
  const [data, setData] = useState([]);
  const [selected, setSelected] = useState(user.redEtilganPageId);

  console.log("RadEtilganContent");
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      try {
        const res = await axiosInstance.post("search/rejected", {
          correspondentName: '',
          shortDescription: '',
          out_number: '',
          out_date: '',
          page: user.redEtilganPageId,
  ...
  }, [currentUser, user.redEtilganPageId]);
  ...
  const linkFunc = useCallback((id) => {
    history.push(`/kiruvchi_bajarish_ijro/${id}/radEtilgan`);
    extensiveSearchBackPageId({ selected: selected, pageName: "redEtilganPageId" })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [history]);

  return (
    <div className="content mb-5 content-mobile">
      <h3 style={{ margin: "10px 0 0 0", fontWeight: "bold", textTransform: "upperCase" }}>Rad Etilgan</h3>
...
export default React.memo(RadEtilganContent);