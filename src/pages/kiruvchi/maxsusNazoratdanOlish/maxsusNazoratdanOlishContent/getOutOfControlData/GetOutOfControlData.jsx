...
const GetOutOfControlData = ({ data, setData, dateFormatSet, selected, setSelected }) => {
  ...
  const linkFunc = (id) => {
    history.push(`/kiruvchi_bajarish_ijro/${id}/maxsusNazoratdanOlish`);
        ...
        <tbody>
          {data?.content?.map((dat, index) => (
            <tr key={index}  style={{ background: dat?.success ? "#BEE8D5" : "" }}
              onClick={() => window.innerWidth <= 768 && history.push(`/kiruvchi_bajarish_ijro/${dat?.id}/nazoratdanOlish`)}>
              ...
              <td style={{ textAlign: "justify", wordWrap: "break-word" }}
                className="qisqacha1 qisqacha mobile-table-none">
...
export default React.memo(GetOutOfControlData);