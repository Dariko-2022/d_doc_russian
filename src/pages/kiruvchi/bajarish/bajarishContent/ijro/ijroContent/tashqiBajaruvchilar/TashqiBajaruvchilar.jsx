...
const TashqiBajaruvchilar = ({ results, setAlert, setResults, currentUser }) => {
            ...
            <div className="modal-body bodyModal" style={{ display: "flex", justifyContent: "center", alignItems: "start", gap: "10px" }}>
              ...
              <ul className="tashqiBaj" style={{ flex: 5 }}>
                {yunalishlar?.length > 0 && yunalishlar.map((dt, index) => (
                  // dt?.organizations?.length > 0 && 
                  <div key={index} className="yunalishlar">
                    <li className="tashqiBajLi1 d-flex justify-content-between" id={dt?.id}  >
                      ...
                      <div className={"text-white fw-bold px-1"} style={
                        { fontSize: '14px', borderRadius: '5px', backgroundColor: "crimson" }
                      }>
...
export default React.memo(TashqiBajaruvchilar);