...
import DoneAllData from "./doneAllData/DoneAllData";
import DoneInputsElements from "./doneAllData/DoneInputsElements";
...
const BajarilganContent = ({ currentUser, permission, ranks }) => {
  const { insta: user } = useSelector(state => state.user);
  const history = useHistory()
  const [data, setData] = useState([]);
  const [selected, setSelected] = useState(user.bajarilganPageId);
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      try {
        const res = await axiosInstance.post("search/done", {
          correspondentName: '',
          shortDescription: '',
          out_number: '',
          out_date: '',
          page: user.bajarilganPageId,
  ...
  }, [currentUser, user.bajarilganPageId]);
  ...
  const linkFunc = useCallback((id) => {
    history.push(`/kiruvchi_bajarish_ijro/${id}/bajarilgan`);
    extensiveSearchBackPageId({ selected: selected, pageName: 'bajarilganPageId' })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [history]);

  return (
    <div className="content mb-5 content-mobile">
      <h3 style={{ margin: "10px 0 0 0", fontWeight: "bold", textTransform: "upperCase" }}>Bajarilgan</h3>
