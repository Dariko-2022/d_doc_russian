...
const DoneAllData = ({ data, linkFunc }) => {
    ...
    <table className="table table-bordered my-1 table-striped table-hover Tab" id="myTable">
      <thead className="theadSticky">
        <tr className="bg-dark text-white NavLink text-center" style={{ height: "45px" }}>
          ...
          <th style={{ width: "10%" }} className="Fayl xujjat mobile-table-none">Fayl</th>
          <th style={{ width: "20%" }} className="korres">Korrespondent</th>
          <th style={{ width: "25%" }} className="qisqacha mobile-table-none">Qisqacha
            Ma'lumot
          </th>
          <th style={{ width: "15%" }} className="reg">Reg № / Muddati</th>
          <th style={{ width: "20%" }} className="ijrochi mobile-table-none">Ijrochi</th>
          <th style={{ width: "5%" }}
            className="text-center harakat mobile-table-none">Harakatlar
          </th>
        </tr>
      </thead>
      <tbody>
        {data.content?.length > 0 && data.content.map((dat, index) => (
          <tr
            key={dat.id}
            onClick={() => window.innerWidth <= 768 && history.push(`/kiruvchi_bajarish_ijro/${dat?.id}/bajarilgan`)}
          >
            <td className="text-center id">{index + 1}</td>
            <td className="text-color Fayl xujjat mobile-table-none" style={{ wordWrap: "break-word" }}>
...
export default React.memo(DoneAllData);