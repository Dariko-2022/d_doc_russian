...
const DeleyInputsElements = ({ setData, setSelected }) => {
  const [startDate, setStartDate] = useState();
  const korrespondent = useRef();
  ...
  const All = useCallback(async () => {
    try {
      ...
      korrespondent.current.value = "";
      shortDescref.current.value = "";
      regNumref.current.value = "";
      setData(res.data);
      setSelected(0);
      extensiveSearchBackPageId({ selected: 0, pageName: "bajarilmaganPageId" })
    } catch (error) {
  ...
  const SearchData = useCallback(async () => {
    ...
    try {
      const res = await axiosInstance.post(`search/notDone`, {
        correspondentName: korrespondent.current.value,
              ...
              <div
                ...
                <div className={'changeBox'} style={{ width: '200px' }}>
                  <DatePicker
                    ...
                    name="dateKechiktirilganKiruvchi"
                  />
                </div>
              </div>
              <div className="form-control-feedback form-control-feedback-lg">
...
export default React.memo(DeleyInputsElements);