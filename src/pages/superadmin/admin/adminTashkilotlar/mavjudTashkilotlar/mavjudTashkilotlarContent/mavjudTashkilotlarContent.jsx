...
const MavjudTashkilotlarContent = (currentUser) => {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      try {
        const res = await axiosInstance.get("organization/checkOrgType")
  ...
  const getTashkilot = async (id) => {
    try {
      const res = await axiosInstance.get("organization/showCorrespondent/byOrgType/" + id)
...
export default React.memo(MavjudTashkilotlarContent);