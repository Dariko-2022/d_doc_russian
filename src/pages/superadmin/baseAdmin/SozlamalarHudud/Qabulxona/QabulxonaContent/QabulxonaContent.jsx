...
const QabulxonaContent = ({ currentUser }) => {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      try {
        const res = await axiosInstanceFq.get('reception')
        if (load)
          setData(res.data);
      } catch (error) {
  ...
  const updatedData = async (obj) => {
    ...
    try {
      const res = await axiosInstanceFq.patch('reception', {
        id: obj.id,
        name: nom
      })
      let arr = [];
  ...
  const changeInputNumber = async (e, id) => {
    if (e.key === "Enter") {
      try {
        const res = await axiosInstanceFq.post(`reception/changeOrder`, {
  ...
  const deletedData = async (id) => {
    try {
      const res = await axiosInstanceFq.delete(`reception/` + id)
      let arr = [];
      data.forEach((d) => {
  ...
  const clickEnter = async () => {
    ...
    try {
      const res = await axiosInstanceFq.post('reception', {
        name: name
      })
...
export default React.memo(QabulxonaContent);