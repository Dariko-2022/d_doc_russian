...
const MurojaatMaqsadiContent = ({ currentUser }) => {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      try {
        const res = await axiosInstanceFq.get('applicationType')
  ...
  const updatedData = async (obj) => {
    ...
    try {
      const res = await axiosInstanceFq.patch('applicationType', {
        id: obj.id,
        name: nom
      })
      let arr = [];
  ...
  const changeInputNumber = async (e, id) => {
    if (e.key === "Enter") {
      try {
        const res = await axiosInstanceFq.post(`applicationType/changeOrder`, {
  ...
  const deletedData = async (id) => {
    try {
      const res = await axiosInstanceFq.delete(`applicationType/` + id)
      let arr = [];
      data.forEach((d) => {
  ...
  const clickEnter = async () => {
    ...
    try {
      const res = await axiosInstanceFq.post('applicationType', {
        name: name
      })
  ...
  return (
    <div className="content mb-5">
      <h3 style={{ margin: "10px 0 0 20px", fontWeight: "bold", textTransform: "upperCase" }}>Murojaat Maqsadi</h3>
...
export default React.memo(MurojaatMaqsadiContent);