...
const MurojaatFormasiContent = ({ currentUser }) => {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      try {
        const res = await axiosInstanceFq.get('applicationForm')
  ...
  const updatedData = async (obj) => {
    ...
    try {
      const res = await axiosInstanceFq.patch('applicationForm', {
        id: obj.id,
        name: nom
      })
      let arr = [];
  ...
  const changeInputNumber = async (e) => {
    if (e.key === "Enter") {
      try {
        const res = await axiosInstanceFq.post(`applicationForm/changeOrder`, {
  ...
  const deletedData = async (id) => {
    try {
      const res = await axiosInstanceFq.delete(`applicationForm/` + id)
      let arr = [];
      data.forEach((d) => {
  ...
  const clickEnter = async () => {
    ...
    try {
      const res = await axiosInstanceFq.post('applicationForm', {
        name: name
      })
  ...
  return (
    <div className="content mb-5">
      <h3 style={{ margin: "10px 0 0 20px", fontWeight: "bold", textTransform: "upperCase" }}>Murojaat Formasi</h3>
...
export default React.memo(MurojaatFormasiContent);