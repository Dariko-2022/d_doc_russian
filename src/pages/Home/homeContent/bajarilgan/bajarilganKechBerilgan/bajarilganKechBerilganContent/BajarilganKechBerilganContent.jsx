...
import DoneDelayAllContent from "./doneDelayAllContent/DoneDelayAllContent";
import DoneDelayInputsElementsHome from "./doneDelayAllContent/DoneDelayInputsElementsHome";

const BajarilganKechBerilganContent = ({ permission, currentUser }) => {
  ...
  useEffect(() => {
    let isMounted = true;
    const getData = async () => {
      const res = await axiosInstance.post(`mainPage/defferedDone/${localStorage.getItem('ids')}`, {
        correspondentName: "",
        shortDescription: "",
        out_number: "",
        out_date: "",
...
export default React.memo(BajarilganKechBerilganContent);