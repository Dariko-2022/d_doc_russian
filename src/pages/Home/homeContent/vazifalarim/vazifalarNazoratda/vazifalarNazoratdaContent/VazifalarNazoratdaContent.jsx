...
import TaskControlData from "./taskControlData/TaskControlData";
import TaskControlInputsElementsHome from "./taskControlData/TaskControlInputsElementsHome";

const VazifalarNazoratdaContent = ({ permission, currentUser }) => {
  ...
  useEffect(() => {
    let isMounted = true;
    const getData = async () => {
      const res = await axiosInstance.post(`mainPage/inControl/${localStorage.getItem('ids')}`, {
        correspondentName: "",
        shortDescription: "",
        out_number: "",
        out_date: '',
...
export default React.memo(VazifalarNazoratdaContent);