...
const TaskContentData = ({ tableData, setTableData, currentUser, permission, dateFormatSet, korresref, shortDescref, regNumref }) => {
  ...
  return (
    <>
      ...
      <table id="myTable" data-paging="true" data-toggle="table" data-search="true"
        className="table table-bordered table-striped table-hover Tab my-1">
        <thead>
          <tr className="bg-dark text-white NavLink text-center" style={{ height: "45px" }}>
            ...
            <th style={{ width: "15%" }} className="ijrochi mobile-table-none">Ijrochi</th>
            <th style={{ width: "5%" }} className="text-center harakat mobile-table-none">Harakatlar</th>
          </tr>
        </thead>
        <tbody>
          {tableData.content?.map((tableItem, index) => (
            <tr
              key={index}
              ...
              <td className="text-color xujjat Fayl mobile-table-none" style={{ wordBreak: "break-all" }}>
                {tableItem?.files?.length > 0 && tableItem?.files?.map((hujjat, i) => (
              ...
              <td className="text-center qiqacha mobile-table-none" style={{ wordBreak: "break-all" }}>{tableItem?.shortDescription}</td>
              ...
              <td className="harakat mobile-table-none">
                <div className="icon d-flex justify-content-center align-items-center ">
                  <Link to={permission ? `/bosh_sahifa_ko'rish/${tableItem?.id}/vazifalarim` : tableItem?.tabName === "SUPER_VISOR" ? `/kiruvchi_bajarish_ijro/${tableItem?.id}/nazorat` : tableItem?.tabName === "GENERALLER" ? `/kiruvchi_bajarish_ijro/${tableItem?.id}/umumlashtiruvchi` : tableItem?.tabName === "FOR_DOING" ? `/kiruvchi_bajarish_ijro/${tableItem?.id}/bajarish` : tableItem?.tabName === "REJECTED" ? `/kiruvchi_bajarish_ijro/${tableItem?.id}/radEtilgan` : tableItem?.tabName === "NOT_DONE" ? `/kiruvchi_bajarish_ijro/${tableItem?.id}/bajarilmagan` : `/kiruvchi_bajarish_ijro/${tableItem?.id}/malumot`}
...
export default React.memo(TaskContentData);