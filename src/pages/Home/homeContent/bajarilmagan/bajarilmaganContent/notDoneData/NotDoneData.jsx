...
const NotDoneData = ({ dateFormatSet, data, setData, permission, currentUser, korresref, shortDescref, regNumref }) => {
  ...
  const handlePageClick = async (e) => {
    ...
    try {
      const res = await axiosInstance.post(`mainPage/allLate/${JSON.parse(localStorage.getItem('ids'))}`, {
  ...
  return (
    <>
      ...
      <table className="table table-bordered my-1 table-striped table-hover Tab" id="myTable">
        ...
        <tbody>
          {data.content?.length > 0 && data.content.map((tableItem, index) => (
            <tr
              key={index}
            ...
            >
              ...
              <td className="text-center ijrochi ijrochi1 mobile-table-none">
                ...
                {tableItem?.inExecutorShortInfoDtos?.length > 0 && tableItem?.inExecutorShortInfoDtos?.map((d, i1) => (
                  d?.isDirect && (
                    <p key={data.content?.length + tableItem?.inExecutorShortInfoDtos?.length + i1}
                      style={{
                        margin: "0",
                        borderColor: "#ddd",
                        height: "100%",
...
export default React.memo(NotDoneData);